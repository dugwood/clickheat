...
for ($i = 0; $i <= $max; $i++)
{
	...
	for ($x = 1; $x < $mx + 1; $x++)
	{
		for ($y = 1; $y < $my + 1; $y++)
		{
			...
			{
				if ($i === $max || imagecolorat($blur, $x + 1, $y) === $previous || imagecolorat($blur, $x - 1, $y) === $previous || imagecolorat($blur, $x, $y + 1) === $previous || imagecolorat($blur, $x, $y - 1) === $previous)
				{
...
for ($x = 1; $x < $mx + 1; $x++)
{
	for ($y = 1; $y < $my + 1; $y++)
	{
		...
		if ($color[0]['red'] + $color[0]['green'] + $color[0]['blue'] === 765)
		{
			...
			$col = imagecolorallocate($img, ceil(($level * $color[0]['red'] + $color[1]['red'] + $color[2]['red'] + $color[3]['red'] + $color[4]['red']) / ($level + 4)), ceil(($level * $color[0]['green'] + $color[1]['green'] + $color[2]['green'] + $color[3]['green'] + $color[4]['green']) / ($level + 4)), ceil(($level * $color[0]['blue'] + $color[1]['blue'] + $color[2]['blue'] + $color[3]['blue'] + $color[4]['blue']) / ($level + 4)));
		}
		else
		{
